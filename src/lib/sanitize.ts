...
export function normalizeDiagram(raw: unknown): TikzSpec | undefined {
  ...
  const code = typeof d.code === 'string' ? d.code : typeof d.tikzCode === 'string' ? d.tikzCode : ''
...
export function sanitizeQuestion(q: any): Omit<QuestionItem, 'id'> {
  ...
  const optionsFromModel = Array.isArray(q.options)
    ? q.options.slice(0, 4).map((x: unknown) => stripOptionPrefix(String(x ?? '').trim())).filter(Boolean)
    : []
  ...
  const diagram = normalizeDiagram(q.diagram) ?? normalizeDiagram({ diagramType: 'tikz', tikzCode: q.tikzCode })
  ...
  const diagramMissing = wantsDiagram && !diagram && !hasSvg ? true : undefined
